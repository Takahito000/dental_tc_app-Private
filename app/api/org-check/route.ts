...
const VALID_CODES = [0, 1, 2, null];
const ANSWER_KEYS = ["q1", "q2", "q3", "q4", "q5", "q6", "q7", "q8", "e1"];
const VALID_ORG_TYPES = ["1", "2", "3", "4", "partial"];

function isValidCode(v: unknown): boolean {
  return (VALID_CODES as unknown[]).includes(v === undefined ? null : v);
}
...
export async function POST(req: Request) {
  ...
  // バリデーション： q1〜q8・e1 は 0/1/2/null のみ、org_type は規定値のみ
  if (
    !answers ||
    typeof answers !== "object" ||
    !ANSWER_KEYS.every((k) => k in answers && isValidCode(answers[k])) ||
  ...
  try {
    ...
    const { error } = await supabase.from("org_check_responses").insert({
      answers: ANSWER_KEYS.reduce<Record<string, unknown>>((acc, k) => {
